...
const FEATURES = [
  {
    ...
    desc: "Drop photos of your clothes. Every garment gets detected and added to your digital wardrobe automatically.",
    color: "bg-neo-blue",
  },
  {
    icon: (
    ...
    title: "Chat Your Style",
    desc: "Describe the occasion and get outfit picks pulled straight from your own wardrobe.",
    color: "bg-neo-accent",
  },
  {
    icon: (
    ...
    desc: "See how the outfit actually looks on you before you step out the door.",
    color: "bg-neo-yellow",
  },
  {
    icon: (
    ...
    title: "Smart Search",
    desc: "Find anything by color, type, or vibe. Just describe what you're looking for.",
    color: "bg-neo-lime",
  },
] as const;

const STEPS = [
  { num: "01", title: "Sign up & snap a selfie", desc: "Your selfie helps us show you how outfits look on you.", accent: "rounded-full bg-neo-accent" },
  { num: "02", title: "Upload your wardrobe", desc: "Drop outfit photos. Every garment gets detected and cataloged.", accent: "bg-neo-yellow" },
  { num: "03", title: "Ask for an outfit", desc: "Say \"date night look\" and get the perfect combo from your closet.", accent: "bg-neo-lime" },
] as const;
...
export default function LandingPage() {
  return (
    <div className="flex min-h-full flex-col">
        ...
        <div className="relative mx-auto flex max-w-5xl flex-col items-center gap-10 text-center">
          ...
          <div className="max-w-3xl animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
            ...
            <p className="mx-auto mt-6 max-w-xl text-base font-medium leading-relaxed text-neo-mute sm:text-lg">
              Upload your clothes, tell us the occasion, and get outfit picks from your own wardrobe. See how they look on you before you head out.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
            ...
            <a href="#features" className="neo-btn bg-neo-surface px-8 py-3.5 text-sm text-neo-ink sm:text-base">See How It Works</a>
          </div>
        </div>
      </section>

      {/* App mockup: Chat view */}
      <section className="px-5 pb-20 sm:px-8">
        <div className="mx-auto max-w-3xl scroll-reveal">
          <BrowserFrame url="styleme.app">
            <MockNav active="chat" />
            <div className="bg-neo-bg p-4" style={{ backgroundImage: "linear-gradient(90deg, color-mix(in srgb, var(--neo-border) 4%, transparent) 1px, transparent 1px), linear-gradient(180deg, color-mix(in srgb, var(--neo-border) 4%, transparent) 1px, transparent 1px)", backgroundSize: "24px 24px" }}>
              ...
              <div className="mb-3 flex justify-end">
                <div className="flex items-start gap-2">
                  <div className="border-2 border-neo-border bg-neo-yellow-soft px-3 py-2 shadow-[2px_2px_0_0_var(--neo-shadow)]">
                    <p className="text-[10px] font-bold text-neo-ink">Something for a summer brunch</p>
                  </div>
              ...
              <div className="mb-3 flex justify-start">
                <div className="flex items-start gap-2">
                  ...
                  <div className="border-2 border-neo-border bg-neo-surface px-3 py-2 shadow-[2px_2px_0_0_var(--neo-shadow)]">
                    <p className="text-[10px] font-medium text-neo-ink">Here&apos;s a light outfit from your wardrobe!</p>
                  </div>
                </div>
              </div>
              {/* Garment match cards */}
              <div className="ml-8 grid grid-cols-3 gap-2">
                {["bg-neo-cyan-soft", "bg-neo-pink-soft", "bg-neo-yellow-soft"].map((bg, i) => (
                  <div key={i} className="border-2 border-neo-border bg-neo-surface shadow-[2px_2px_0_0_var(--neo-shadow)]">
                    <div className={`aspect-square w-full ${bg}`} />
                    <div className="border-t border-neo-border p-1.5">
                      <div className="h-1.5 w-3/4 bg-neo-ink" style={{ opacity: 0.2 }} />
                      <div className="mt-1 h-1 w-1/2 bg-neo-mute" style={{ opacity: 0.15 }} />
                    </div>
                  </div>
                ))}
              </div>
              <div className="ml-8 mt-1 text-[7px] font-extrabold uppercase tracking-widest text-neo-mute">3 pieces selected</div>
              {/* Try-on preview */}
              <div className="ml-8 mt-3 border-2 border-neo-border bg-neo-surface shadow-[3px_3px_0_0_var(--neo-shadow)]">
                <div className="flex aspect-16/10 w-full items-center justify-center bg-neo-lavender texture-crosshatch">
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <div className="h-10 w-7 border-2 border-neo-border bg-neo-pink-soft" />
                    <div className="h-6 w-9 border-2 border-neo-border bg-neo-cyan-soft" />
                  </div>
                </div>
                <div className="flex items-center gap-1.5 border-t border-neo-border px-2 py-1.5">
                  ...
                  <span className="text-[7px] font-extrabold uppercase tracking-widest text-neo-mute">Try-on preview</span>
                </div>
              </div>
              {/* Composer bar */}
              <div className="mt-4 flex gap-2">
                <div className="flex h-8 flex-1 items-center border-2 border-neo-border bg-neo-surface px-2 shadow-[2px_2px_0_0_var(--neo-shadow)]">
                  <span className="text-[8px] font-bold uppercase text-neo-mute" style={{ opacity: 0.4 }}>Describe your outfit...</span>
                </div>
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="mb-14 max-w-xl scroll-reveal">
            ...
            <p className="mt-4 text-sm font-medium leading-relaxed text-neo-mute sm:text-base">
              From uploading photos to getting dressed, StyleMe handles the entire flow.
            </p>
          </div>

          <div className="stagger grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f) => (
              <div key={f.title} className={`neo-card flex flex-col gap-4 p-6 border-t-4 ${f.color.replace("bg-", "border-t-")}`}>
                <div className={`flex h-14 w-14 items-center justify-center border-3 border-neo-border ${f.color} text-white shadow-[3px_3px_0_0_var(--neo-shadow)] hover-jelly`}>
                  {f.icon}
                </div>
          ...
          <BrowserFrame url="styleme.app/wardrobe">
            ...
            <div className="bg-neo-bg p-4" style={{ backgroundImage: "linear-gradient(90deg, color-mix(in srgb, var(--neo-border) 4%, transparent) 1px, transparent 1px), linear-gradient(180deg, color-mix(in srgb, var(--neo-border) 4%, transparent) 1px, transparent 1px)", backgroundSize: "24px 24px" }}>
              ...
              <div className="border-2 border-neo-border bg-neo-surface p-3 shadow-[3px_3px_0_0_var(--neo-shadow)]">
                ...
                {/* Category pills */}
                <div className="mt-2 flex gap-1 border-t border-neo-border pt-2">
                  {["Shirt", "Pants", "Jacket", "Shoes"].map((c, i) => (
              ...
              <div className="mt-3 grid grid-cols-4 gap-2 sm:grid-cols-5">
                {[
                  { top: "bg-neo-accent", bg: "bg-neo-pink-soft", label: "Shirt" },
                  { top: "bg-neo-blue", bg: "bg-neo-cyan-soft", label: "Jeans" },
                  { top: "bg-neo-yellow", bg: "bg-neo-yellow-soft", label: "Jacket" },
                  { top: "bg-neo-lime", bg: "bg-neo-lime-soft", label: "Sneakers" },
                  { top: "bg-neo-accent", bg: "bg-neo-peach", label: "Dress" },
                  { top: "bg-neo-blue", bg: "bg-neo-cyan-soft", label: "Shorts" },
                  { top: "bg-neo-yellow", bg: "bg-neo-yellow-soft", label: "Hat" },
                  { top: "bg-neo-lime", bg: "bg-neo-lime-soft", label: "Bag" },
                ].map((item, i) => (
                  <div key={i} className="border-2 border-neo-border bg-neo-surface shadow-[2px_2px_0_0_var(--neo-shadow)]">
      ...
      {/* How It Works */}
      <section className="relative border-y-3 border-neo-border bg-neo-surface px-5 py-20 sm:px-8 sm:py-28">
          ...
          <div className="stagger grid gap-6 sm:grid-cols-3">
            {STEPS.map((s, i) => (
              <div key={s.num} className="group relative flex flex-col gap-4 border-3 border-neo-border bg-neo-surface p-6 shadow-[4px_4px_0_0_var(--neo-shadow)] transition-all duration-200 hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_0_var(--neo-shadow)]">
                ...
                <span className="text-6xl font-black uppercase text-neo-mute sm:text-7xl" style={{ opacity: 0.12 }}>{s.num}</span>
                ...
                {i === 2 && (
                  <div className="mt-1 flex items-center gap-3">
                    ...
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--neo-mute)" strokeWidth="2.5" strokeLinecap="round"><line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" /></svg>
                    <div className="flex gap-1">
                      <div className="h-6 w-6 border border-neo-border bg-neo-pink-soft" />
                      <div className="h-6 w-6 border border-neo-border bg-neo-cyan-soft" />
                    </div>
                  </div>
                )}
                {i < STEPS.length - 1 && (
                  <div className="absolute -right-4 top-1/2 hidden text-neo-mute sm:block">
      ...
      <section className="relative overflow-hidden border-t-3 border-neo-border px-5 py-20 sm:px-8 sm:py-28">
        ...
        <div className="relative mx-auto flex max-w-3xl flex-col items-center gap-8 text-center scroll-reveal">
          ...
          <p className="max-w-md text-sm font-medium leading-relaxed text-neo-mute sm:text-base">Upload your first outfit and let AI handle the rest.</p>
      ...
      <footer className="border-t-3 border-neo-border bg-neo-surface px-5 py-8 sm:px-8">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 sm:flex-row">
          ...
          <p className="text-xs font-bold uppercase tracking-wider text-neo-mute">Built with AI, designed for your closet</p>
